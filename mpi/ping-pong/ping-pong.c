  ...
  /* COMPLETE: loop of NREPS iterations.
     ...
     for (int i=0 ; i<NREPS; i++) {
     	if (myid == 0) { //soy el 0
     		...
     		//MPI_Recv(buf, n, MPI_BYTE, 1, 100, MPI_COMM_WORLD, &status);
     		MPI_Sendrecv(buf, n, MPI_BYTE, 1, 100, buf, n, MPI_BYTE, 1, 100, MPI_COMM_WORLD, &status);
     	}
     	else if (myid == 1) { //soy el 1
     		//MPI_Recv(buf, n, MPI_BYTE, 0, 100, MPI_COMM_WORLD, &status);
     		//MPI_Send(buf, n, MPI_BYTE, 0, 100, MPI_COMM_WORLD);
     		MPI_Sendrecv(buf, n, MPI_BYTE, 0, 100, buf, n, MPI_BYTE, 0, 100, MPI_COMM_WORLD, &status);
     	}
     }
