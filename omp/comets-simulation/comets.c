...
int main(int argc, char **argv)
{
    ...
    for (step = 0; step < STEPS; ++step) {
        #pragma omp parallel for default(none) private(i,rx,ry,r2,r,inv_r3,ax,ay,v2,v_kms,bin) shared(x,y,vx,vy,N,dt,hist,grazing,grazing_count,step) reduction(+:total_kinetic_energy)
        for (i = 0; i < N; ++i) {
            rx = x[i]; 
			ry = y[i];
            r2 = rx*rx + ry*ry;
